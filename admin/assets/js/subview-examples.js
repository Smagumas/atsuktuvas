	...
	var runEventFormValidation = function(el) {
		...
		formEvent.validate({
			...
			submitHandler : function(form) {
				...
				errorHandler2.hide();
				newEvent = new Object;
				newEvent.title = $(".form-event .event-name ").val(), newEvent.start = new Date($('.form-event .event-start-date').val()), newEvent.end = new Date($('.form-event .event-end-date').val()), newEvent.allDay = $(".form-event .all-day").bootstrapSwitch('state'), newEvent.className = $(".form-event .event-categories option:checked").val(), newEvent.category = $(".form-event .event-categories option:checked").text(), newEvent.content = $eventDetail.code();
